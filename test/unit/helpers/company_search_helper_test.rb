...
class CompanySearchHelperTest < ActionView::TestCase
  ...
  test "search without companies" do
    Company.delete_all
    assert_equal({:data => [""], :query => "Super", :suggestions => ["Super"]}, CompanySearchHelper.search_for_companies("Super"))
  end
  ...
  test "search with string from the middle of the name" do
    company = Company.create!(default_hash(Company, :name => "Super Company"))
    Contact.create!(default_hash(Contact, :company => company))
    assert_equal({:query => "pa", :suggestions => ["Super Company", "pa"], :data => [company.id, ""]}, CompanySearchHelper.search_for_companies("pa"))
  end

  test "search with different string case" do
    company = Company.create!(default_hash(Company, :name => "Super Company"))
    Contact.create!(default_hash(Contact, :company => company))
    assert_equal({:query => "MPA", :suggestions => ["Super Company", "MPA"], :data => [company.id, ""]}, CompanySearchHelper.search_for_companies("MPA"))
  end

  test "search with no matching string" do
    company = Company.create!(default_hash(Company))
    Contact.create!(default_hash(Contact, :company => company))
    assert_equal({:query => "Macro", :suggestions => ["Macro"], :data => [""]}, CompanySearchHelper.search_for_companies("Macro"))
  end
  ...
  test "search with two companies" do
    ...
    assert_equal({:query => "Co", :suggestions => ["Macro Corp", "Super Company", "Co"], :data => [company1.id, company2.id, ""]}, CompanySearchHelper.search_for_companies("Co"))
  end
  ...
  test "search with two companies returning only one" do
    ...
    assert_equal({:query => "Macro", :suggestions => ["Macro Corp", "Macro"], :data => [company.id, ""]}, CompanySearchHelper.search_for_companies("Macro"))
  end
end
