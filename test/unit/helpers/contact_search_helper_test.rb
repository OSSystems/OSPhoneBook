...
class ContactSearchHelperTest < ActionView::TestCase
  ...
  test "search with one contact" do
    ...
    assert_equal({:query => "John", :suggestions => ["John Doe"], :data => [contact.id]}, ContactSearchHelper.search_for_contacts("John"))
  end

  test "search with string from the middle of the name" do
    contact = Contact.create!(default_hash(Contact))
    assert_equal({:query => "oh", :suggestions => ["John Doe"], :data => [contact.id]}, ContactSearchHelper.search_for_contacts("oh"))
  end

  test "search with different string case" do
    contact = Contact.create!(default_hash(Contact))
    assert_equal({:query => "DOE", :suggestions => ["John Doe"], :data => [contact.id]}, ContactSearchHelper.search_for_contacts("DOE"))
  end
  ...
  test "search with two contacts" do
    ...
    assert_equal({:query => "Doe", :suggestions => ["Jane Doe", "John Doe"], :data => [contact1.id, contact2.id]}, ContactSearchHelper.search_for_contacts("Doe"))
  end
  ...
  test "search with two contacts returning only one" do
    ...
    assert_equal({:query => "Jane", :suggestions => ["Jane Doe"], :data => [contact.id]}, ContactSearchHelper.search_for_contacts("Jane"))
  end
  ...
  test "search using tag" do
    ...
    contact.save!
    contact.reload
    assert_equal({:query => "Abnormals", :suggestions => ["Jane Doe"], :data => [contact.id]}, ContactSearchHelper.search_for_contacts("Abnormals"))
  end
  ...
  test "search using two tags" do
    ...
    contact.save!
    contact.reload
    assert_equal({:query => "Ab", :suggestions => ["Jane Doe"], :data => [contact.id]}, ContactSearchHelper.search_for_contacts("Ab"))
  end
  ...
  test "search matching name and tags" do
    ...
    contact.save!
    contact.reload
    assert_equal({:query => "a", :suggestions => ["Jane Doe"], :data => [contact.id]}, ContactSearchHelper.search_for_contacts("a"))
  end
